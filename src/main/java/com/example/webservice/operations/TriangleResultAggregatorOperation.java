...
public class TriangleResultAggregatorOperation {
    public static Collection<TriangleMemberStatistic> aggregate(
        ...
        var areaResults = results.stream().map(res -> res.area).collect(Collectors.toCollection(ArrayList::new));
        var perimeterResults = results.stream().map(res -> res.perimeter).collect(Collectors.toCollection(ArrayList::new));

        var side1 = entities.stream().map(side -> side.A).collect(Collectors.toCollection(ArrayList::new));
        var average1 = side1.stream().mapToDouble(Double::doubleValue).average().getAsDouble();

        var side2 = entities.stream().map(side -> side.B).collect(Collectors.toCollection(ArrayList::new));
        var average2 = side2.stream().reduce(Double::sum).get() / side2.size();

        var side3 = entities.stream().map(side -> side.C).collect(Collectors.toCollection(ArrayList::new));
